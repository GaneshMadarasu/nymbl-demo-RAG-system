...
Two markup workflows are not yet covered. **(Path 2) Print → handwrite → scan to PDF.** The pen strokes become pixels in a page raster with no annotation objects; PyMuPDF can't see them as text or markup. Recovering them needs Vision OCR on the page image, but the existing OCR-scanned-pages toggle only fires when PyMuPDF returns ≤ 50 chars — a typed PDF with handwritten margin notes printed on top will skip OCR. An "always-OCR" toggle (or the image-aware threshold path described in the OCR limitations below) would close this. **(Path 3) Hand-write directly on a typed digital PDF, exported as flattened PDF (e.g., iPad Notes save).** The ink merges into the page raster; same Vision-OCR-everything constraint as Path 2. Ink annotations (`/Ink` subtype) on layered PDFs would also benefit from the same Vision pass to recognise stroke content as text. None of these are built today — Path 1 (annotation objects) is the most common workflow and is the case currently supported.
