...
**Visual-markup detection for flattened ink (opt-in)**
PDF annotation reading covers the *layered* case (markup is its own object with metadata). When a user prints, marks up by hand, and scans back to PDF — or hand-writes on a typed PDF in an iPad app that flattens ink into the page raster — the annotation objects don't exist; the underline is just pixels. When the **"Hand Written?"** toggle is enabled at upload, `_collect_visual_markup` runs a Vision pass on every page that has body text, asking the model to identify hand-drawn underlines, highlights, circles, arrows, stars, margin notes, and strikethroughs and return structured JSON `[{type, color, text}, ...]`. Each detected mark is appended to the page text as `[underline in red: deadline]` / `[note in blue: review]` — same bracketed shape as the layered annotation tags, so the chunker, retriever, and answer LLM see one uniform format regardless of source. Cost: one Vision call per page that has text, capped at `_GEMINI_CONCURRENCY=8` in flight; for a 100-page doc that's ~12 batches. The prompt explicitly excludes typographic emphasis (bold, italic, typeset rules) so the model only reports actual ink-stroke markup.
